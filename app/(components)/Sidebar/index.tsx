...
const SidebarLink = ({
    ...
    return (
        <Link href={href}>
            ...
            `}>
                <Icon size={24} className='w-6 h-6 !text-gray-700' />
                <span className={`${isCollapsed ? 'hidden' : 'block'} font-semibold`}>{label}</span>
...
const Sidebar = () => {
    ...
    const sidebarClassNames = `fixed flex flex-col ${
        isSidebarCollapsed ? "w-0 md:w-16" : "w-72 md:wd-64"
  ...
  return (
    <div className={sidebarClassNames}>
        ...
        <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
            <div>logo</div>
            <h1 className={`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-xl`}>IMS</h1>
        ...
        <footer className={`${isSidebarCollapsed ? 'hidden' : 'block'} mb-10`}>
            <p className='text-center text-xs text-gray-500'>&copy; 2025 Inventory Management System</p>
        </footer>
